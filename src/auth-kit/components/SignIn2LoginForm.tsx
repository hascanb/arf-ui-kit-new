...
export function SignIn2LoginForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!username.trim() || !password) {
      setError(tOr('validation.required', 'Lutfen tum alanlari doldurun'))
      return
    }
    ...
    try {
      ...
      if (!response.success) {
        setError(sanitizeError(response.error, tOr('errors.generic', 'Giris basarisiz')))
        return
      }
      ...
      <FieldGroup>
        <div className="flex flex-col items-start gap-1 text-left">
          <h1 className="text-2xl font-semibold tracking-tight text-slate-900">
            {tOr('signIn.title', 'Giris Yap')}
          </h1>
          <p className="text-sm text-balance text-slate-600">
            {tOr('signIn.subtitle', 'Hesabiniza giris yapin')}
          </p>
        </div>
        ...
        <Field>
          <FieldLabel htmlFor="username">{tOr('signIn.username', 'Kullanici Adi')}</FieldLabel>
          <Input
            id="username"
            type="text"
            value={username}
        ...
        <Field>
          <div className="flex items-center">
            <FieldLabel htmlFor="password">{tOr('signIn.password', 'Sifre')}</FieldLabel>
        ...
        {(canUseGoogle || canUseApple) && (
          <>
            <FieldSeparator>{tOr('signIn.orContinueWith', 'veya')}</FieldSeparator>
            <Field>
